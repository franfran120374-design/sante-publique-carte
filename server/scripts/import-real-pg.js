...
async function importFINESS(pool, filePath) {
    if (!fs.existsSync(filePath)) { console.log('FINESS: fichier absent'); return 0; }
    ...
    console.log(`\nFINESS: ${imported} importés (${skipped} skip)`);
    return imported;
}

async function importAmeli(pool, filePath) {
    if (!fs.existsSync(filePath)) { console.log('AMELI: fichier absent'); return 0; }
    let imported = 0, skipped = 0, batch = [];
    ameliIdx = 0;
    const rl = readline.createInterface({ input: fs.createReadStream(filePath, { encoding: 'utf8' }), crlfDelay: Infinity });
    ...
    for await (const line of rl) {
        const c = line.split(';');
        if (c.length < 22) { skipped++; continue; }
        ...
        batch.push([
            `AMELI-${ameliIdx}`, nom, prenom, specialite, specialite, secteur,
            carteVitale === 'true' ? 1 : 0, adresse, codePostal, ville, departement,
            coords.lat + jitter(), coords.lng + jitter()
        ]);
        imported++;
        ...
        if (batch.length >= BATCH_SIZE) {
            ...
            process.stdout.write(`\rAMELI: ${imported} importés...`);
            batch = [];
        }
    }
    if (batch.length > 0) {
    ...
    console.log(`\nAMELI: ${imported} importés (${skipped} skip)`);
    return imported;
}
...
async function main() {
    ...
    await importFINESS(pool, path.join(dlDir, 'finess.csv'));
    await importAmeli(pool, path.join(dlDir, 'ameli-ps.csv'));
