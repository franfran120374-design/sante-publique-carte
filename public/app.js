...
function isNearCenter(lat, lng, dept) {
    const c = DEPT_CENTER_MAP[dept];
    if (!c) return false;
    return Math.abs(lat - c.lat) < 0.02 && Math.abs(lng - c.lng) < 0.02;
}

async function autoGeocode(table, items) {
    const wrongIds = items.filter(i => i.latitude && i.longitude && isNearCenter(i.latitude, i.longitude, i.departement)).map(i => i.id);
    if (wrongIds.length === 0) return;

    for (let i = 0; i < wrongIds.length; i += 20) {
        const batch = wrongIds.slice(i, i + 20);
        try {
            await fetch(`${API}/api/geocode/batch`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ids: batch, table })
            });
        } catch (e) { /* silent */ }
    }
}
